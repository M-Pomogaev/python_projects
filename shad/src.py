...
def weighted_impurity(y_left: np.ndarray, y_right: np.ndarray) -> \
    ...
    sum = y_left.shape[0] + y_right.shape[0]
    weighted_impurity = (y_left.shape[0] / sum) * left_impurity + \
                        (y_right.shape[0] / sum) * right_impurity
...
def _best_split(self, X: np.ndarray, y: np.ndarray):
    ...
    for feature in features:
        ...
        for threshold in thresholds:
            left_ind, right_ind = create_split(current_feature_values, threshold)
            current_weighted_impurity, current_left_impurity, current_right_impurity = \
                weighted_impurity(y[left_ind], y[right_ind])
...
class MyDecisionTreeNode:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        N = y.size
        if (
            self._meta.max_depth <= self._depth or self._meta.min_samples_split > N or len(count) == 1
        ):
            self._node_type = NodeType.TERMINAL
            self._predicted_class = np.argmax(np.bincount(y))
            self._class_proba = np.bincount(y, minlength=self._meta._n_classes)/N
            return self
...
class MyRandomForestClassifier:
    ...
    def _bootstrap_sample(self, X: np.ndarray, y: np.ndarray):
        ...
        :return:
        """
        indices = np.random.randint(low=0, high=X.shape[0], size=max((X.shape[0]//2), 2))
...
y = np.array([0, 1, 2, 3])
rf = MyRandomForestClassifier(n_estimators=100).fit(X, y)
...
class ExperimentHandler:
    ...
    def score_cv(self, estimator, metrics, run):
        ...
        :return: None
        """
        cross_val_results = cross_validate(estimator, self.X_train, self.y_train, scoring=metrics, cv=self._cv_iterable, n_jobs=self._n_jobs)
    ...
    def generate_stacking_predictions(self, estimator, run):
        ...
        if hasattr(estimator, "predict_proba"):  # choose the most informative method for stacking predictions
            ...
            method = "predict"
        cross_val_stacking_prediction = cross_val_predict(estimator, self.X_train, self.y_train, cv=self._cv_iterable, n_jobs=self._n_jobs, method=method)
